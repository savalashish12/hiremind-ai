...
const AdminDashboard = () => {
  ...
  const [logPagination, setLogPagination] = useState({ total: 0, page: 1, limit: 25, pages: 1 });
  ...
  const LOG_ACTIONS = ["ALL", "LOGIN", "REGISTER", "JOB_CREATED", "JOB_DELETED", "APPLICATION_SUBMITTED", "INTERVIEW_SCHEDULED", "OFFER_LETTER_GENERATED", "OFFER_ACCEPTED", "OFFER_DECLINED", "PAYMENT_SUBMITTED", "PAYMENT_APPROVED", "PAYMENT_REJECTED", "USER_SUSPENDED", "USER_ACTIVATED", "USER_DELETED"];
  ...
  const fetchLogs = async (page = logPage) => {
    try {
      ...
      if (logFrom) params.startDate = logFrom;
      if (logTo) params.endDate = logTo;
      const res = await API.get("/admin/activity-logs", { params });
      // Back-compat: old API returned a bare array
      if (Array.isArray(res.data)) {
        setLogs(res.data);
        setLogPagination({ total: res.data.length, page: 1, limit: 25, pages: 1 });
      } else {
        setLogs(res.data.logs || []);
        setLogPagination(res.data.pagination || { total: 0, page: 1, limit: 25, pages: 1 });
      }
    } catch {
  ...
  const exportLogsCsv = () => {
    ...
    const header = ["Timestamp", "User", "Email", "Role", "Action", "Entity", "Details", "IP"];
    ...
    a.href = url;
    a.download = `activity-logs-page${logPagination.page}.csv`;
    a.click();
            ...
            >
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-slate-850">
                <div>
                  ...
                  <p className="text-xs text-slate-455 mt-1">
                    {logPagination.total} events · page {logPagination.page} of {logPagination.pages || 1}
                  </p>
                </div>
                <button
                  onClick={exportLogsCsv}
                  className="flex items-center gap-1.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 font-bold px-3.5 py-2 rounded-xl text-xs transition-all"
                >
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                ...
                <select
                  ...
                  className="bg-slate-950 border border-slate-850 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-blue-500"
                >
                  {LOG_ACTIONS.map((a) => <option key={a} value={a}>{a === "ALL" ? "All actions" : a}</option>)}
                </select>
                <input
                  type="date"
                  value={logFrom}
              ...
              <div>
                <button
                  ...
                  className="bg-blue-600 hover:bg-blue-500 text-white font-bold px-5 py-2 rounded-xl text-xs transition-all"
                >
                  Apply Filters
                </button>
              </div>
                  ...
                  <table className="w-full text-left border-collapse text-xs min-w-[800px]">
                    <thead>
                      <tr className="bg-slate-950 text-slate-400 font-semibold sticky top-0 z-15 shadow">
                        <th className="p-4 border-b border-slate-850">Timestamp</th>
                        <th className="p-4 border-b border-slate-850">User</th>
                        <th className="p-4 border-b border-slate-850">Action Event Code</th>
                        <th className="p-4 border-b border-slate-850">Entity</th>
                    ...
                    <tbody className="divide-y divide-slate-855 text-slate-350">
                      {logs.length === 0 ? (
                        <tr>
                          <td colSpan="5" className="text-center py-10 text-slate-500 italic">No activity audit logs found matching criteria.</td>
                        </tr>
                      ) : (
                        logs.map((log) => (
                          <tr key={log.id} className="hover:bg-slate-900/30 transition-colors">
                            ...
                            <td className="p-4">
                              <p className="font-bold text-slate-200">{log.user?.fullName || "—"}</p>
                              <p className="text-[10px] text-slate-500">{log.user?.email || log.userId.slice(0, 8)}</p>
                              {log.user?.role && (
                                <span className="inline-block mt-0.5 px-1.5 py-px rounded text-[9px] font-bold bg-slate-800 text-slate-300 border border-slate-700">{log.user.role}</span>
                              )}
                            </td>
                            <td className="p-4 font-bold">
                              <span className={`px-2 py-0.5 rounded text-[9px] font-extrabold uppercase tracking-wide border whitespace-nowrap ${
                                log.action.includes('CREATED') || log.action.includes('APPROVED') || log.action === 'LOGIN' ? 'bg-green-500/5 text-green-400 border-green-500/10' :
                                log.action.includes('DELETED') || log.action.includes('REJECTED') || log.action.includes('SUSPENDED') ? 'bg-red-500/5 text-red-400 border-red-500/10' :
                                log.action.includes('SCHEDULED') || log.action.includes('SUBMITTED') ? 'bg-yellow-500/5 text-yellow-400 border-yellow-500/10' : 'bg-purple-500/5 text-purple-400 border-purple-500/10'
                              }`}>
                                {log.action}
                              </span>
                            </td>
                            <td className="p-4 text-slate-400">{log.entity || "—"}</td>
                            <td className="p-4 text-slate-200 leading-normal max-w-[320px]">{log.details}</td>
                          </tr>
                        ))
                      )}
                    </tbody>
              ...
              <div className="flex items-center justify-between text-xs text-slate-400">
                <span>{logPagination.total} total events</span>
                <div className="flex gap-2">
                  <button
                    disabled={logPagination.page <= 1}
                    onClick={() => { const p = logPagination.page - 1; setLogPage(p); fetchLogs(p); }}
                    className="bg-slate-800 hover:bg-slate-700 disabled:opacity-40 border border-slate-700 px-3 py-1.5 rounded-xl font-bold"
                  >
                    ← Prev
                  </button>
                  <span className="px-2 py-1.5 font-mono">Page {logPagination.page} / {logPagination.pages || 1}</span>
                  <button
                    disabled={logPagination.page >= (logPagination.pages || 1)}
                    onClick={() => { const p = logPagination.page + 1; setLogPage(p); fetchLogs(p); }}
                    className="bg-slate-800 hover:bg-slate-700 disabled:opacity-40 border border-slate-700 px-3 py-1.5 rounded-xl font-bold"
                  >
                    Next →
